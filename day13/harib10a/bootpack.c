...
void HariMain(void)
{
  ...
  for(;;) {
	...
	putfonts8_asc_sht(sht_win, 40, 28, COL8_000000, COL8_C6C6C6, s, 15);

     io_cli();
	int key_status = 0, mouse_status = 0, timer_status = 0;
	key_status = fifo8_status(&keyfifo);
	mouse_status = fifo8_status(&mousefifo);
	timer_status = fifo8_status(&timerfifo);

     if(0 == key_status + mouse_status + timer_status) {
	 			io_sti();
    	}else{
     	if(0 != key_status) {
	 			i = fifo8_get(&keyfifo);
			...
			if (10 == i) {
				...
				if (0 != i) {
					timer_init(timer3, &timerfifo, 0); // set to 0
